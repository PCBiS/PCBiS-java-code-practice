...
public class InfoController extends InfoControllerAbstract implements AddInterface {
	...
	private InfoController() {
		this.pInfo = new ArrayList<PhoneInfor>(); // 배열 크기는 100개.
		//this.pIdx = 0; // 숫자는 0 to 99(100개), 이 이상 증가하려고 하면 막는 로직이 존재해야 함, 0.6 버전에서는 해당 부분이 필요 없음 			
	...
	public void infoSearch(String searchWord) {
		int findIdx = -1;
		if (pInfo.size()<1) {System.out.println("사용자가 아무도 등록 되어있지 않습니다!");}		
		else{
			System.out.println("debug!! searchWord : " + searchWord);
			findIdx = searchIdx(searchWord);
			System.out.println("debug!! findIdx : " + findIdx);
			if (findIdx<0) {
	...
	public void infoDelete(String searchWord) {
		int findIdx = -1;
		if (pInfo.size()<0) {System.out.println("사용자가 아무도 등록 되어있지 않습니다!");}
		else {
			findIdx = searchIdx(searchWord);
			if (findIdx<0) {
				System.out.println("검색 결과 일치하는 이름의 사용자를 찾지 못했습니다.");
	...
	public void infoModify(String searchWord) {
		...
		if (pInfo.size()<0) {System.out.println("사용자가 아무도 등록 되어있지 않습니다!");}
			...
			if (findIdx<0) {
				...
				System.out.println("============================================");
				if (pInfo.get(findIdx) instanceof PhoneCorpInfor) {
					modifyInfo(AddInterface.Work, findIdx);
				}else if (pInfo.get(findIdx) instanceof PhoneUnivInfor) {
					modifyInfo(AddInterface.Univ, findIdx);
				}else if (pInfo.get(findIdx) instanceof PhoneClubInfor) {
					modifyInfo(AddInterface.Club, findIdx);
				}else if (pInfo.get(findIdx) instanceof PhoneInfor) {
	...
	private int searchIdx(String keyword) {
		int findIdx =-1;
		for (int i = 0; i < pInfo.size(); i++) {
			if (pInfo.get(i).name.equals(keyword)) {
				findIdx = i;
				break;
			}
		}
	...
	private void showInfoList(int select) {		
	...
	private void addBaseInfo(String name, String phoneNumber, String address, String email, String birthDay) {
		if (this.pInfo.size()>99) {
			System.out.println("더 이상 저장 할 수가 없습니다. 처음으로 돌아갑니다.");
		}else {			
			this.pInfo.add(new PhoneInfor(name, phoneNumber, address, email, birthDay));			
			addComplete();
			}
		}
	...
	private void addBaseInfo(String name, String phoneNumber, String address, String email, String birthDay, int pIdx) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.remove(pIdx);
			this.pInfo.add(pIdx, new PhoneInfor(name, phoneNumber, address, email, birthDay));			
			addComplete();
			}
		}	
	...
	private void addCorpInfo(String name, String phoneNumber, String address, String email, String birthDay, String company) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.add(new PhoneCorpInfor(name, phoneNumber, address, email, birthDay, company));
			addComplete();
		}
	}
	...
	private void addCorpInfo(String name, String phoneNumber, String address, String email, String birthDay, String company, int pIdx) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.remove(pIdx);
			this.pInfo.add(pIdx, new PhoneCorpInfor(name, phoneNumber, address, email, birthDay, company));		
			addComplete();
		}
	}
	...
	private void addUnivInfo(String name, String phoneNumber, String address, String email, String birthDay, String major, String year) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.add(new PhoneUnivInfor(name, phoneNumber, address, email, birthDay, major, year));				
			addComplete();
		}
	}
	...
	private void addUnivInfo(String name, String phoneNumber, String address, String email, String birthDay, String major, String year, int pIdx) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.remove(pIdx);
			this.pInfo.add(pIdx, new PhoneUnivInfor(name, phoneNumber, address, email, birthDay, major, year));			
			addComplete();
		}
	}
	...
	private void addClubInfo(String name, String phoneNumber, String address, String email, String birthDay, String clubNickName, String clubRole) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.add(new PhoneClubInfor(name, phoneNumber, address, email, birthDay, clubNickName, clubRole));				
			addComplete();
		}
	}
	...
	private void addClubInfo(String name, String phoneNumber, String address, String email, String birthDay, String clubNickName, String clubRole, int pIdx) {
		if (this.pInfo.size()>99) {
			...
			this.pInfo.remove(pIdx);
			this.pInfo.add(pIdx, new PhoneClubInfor(name, phoneNumber, address, email, birthDay, clubNickName, clubRole));						
			addComplete();
		}
	}
