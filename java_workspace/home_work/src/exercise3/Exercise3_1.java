...
public class Exercise3_1 {
	...
	public static void main(String[] args) {
		...
		// 1. x: 2, y: 15, 출력값 13 - 이유는 y=5에 10을 증가한 값을 대입 한 상태 y=15에서 x의 값 2를 뺀 값임. 
		System.out.println(x += 2);
		// 2. x: 5, y: 15, 출력값 5 - 직전연산(x=2)에서 후위연산으로 1이 증가(x=3)한 값에 2를 다시 증가(x=5)시키고 그 값을 출력했기 때문.
		System.out.println(!('A' <= c && c <= 'Z')); 
		// 3. 'A' 의 Ascii 코드로서의 숫자는 65인데 'c' 의 Ascii코드 값은 99이기 때문에 65 <= 99 = false 이기 때문에 false 가 나와서 추가 연산 없이 종료.
